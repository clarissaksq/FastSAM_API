...
class FastSAMPrompt:
    ...
    def text_prompt_old(self, text, rank):
        if self.results == None:
            return []
    ...
    def text_prompt_threshold(self, text, threshold):
        ...
        threshold_value = high_score * threshold

        masks = []
        for idx, score in enumerate(scores):
            if score > threshold_value:
                idx_adjusted = idx + sum(np.array(filter_id) <= int(idx))
                masks.append(np.array(annotations[idx_adjusted]['segmentation']))
        return masks
    
    def text_prompt_dbscan(self, text, eps=0.0000000000000000001):  
        if self.results is None:
            return []
        ...
        clustering = DBSCAN(eps=eps).fit(scores)
        labels = clustering.labels_

        unique_labels = set(labels)
        if -1 in unique_labels:
            unique_labels.remove(-1)  
        first_cluster_label = min(unique_labels) if unique_labels else None

        masks = []
        if first_cluster_label is not None:
            for idx, label in enumerate(labels):
                if label == first_cluster_label:
                    idx_adjusted = idx + sum(np.array(filter_id) <= int(idx))
                    masks.append(np.array(annotations[idx_adjusted]['segmentation']))
